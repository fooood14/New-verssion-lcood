...
import { LogOut, Settings, BookOpen, Clock, Play, ShieldCheck } from 'lucide-react';
...
const PermanentExamsPage = () => {
  ...
  const handleDashboardLogin = () => {
    if (pin === 'lcood2285') {
      ...
      toast({ title: "تم التحقق بنجاح", description: "مرحباً بك في لوحة التحكم!" });
      navigate('/dashboard');
    } else {
      toast({
        title: "خطأ في التحقق",
  ...
  const handleKeyPress = (e) => {
    if (e.key === 'Enter') handleDashboardLogin();
  };
  ...
  return (
    <div className="min-h-screen px-4 py-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">الاختبارات المتاحة</h1>
          <p className="text-slate-400 text-sm md:text-base">مرحباً بك، {user?.email}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button
            variant="outline"
            onClick={handleDashboardAccess}
            className="text-slate-300 border-slate-600 hover:bg-slate-700 hover:text-blue-400 text-sm md:text-base"
          >
          ...
          <Button
            ...
            variant="outline"
            className="text-slate-300 border-slate-600 hover:bg-slate-700 hover:text-red-400 text-sm md:text-base"
          >
      ...
      >
        <div className="text-center mb-8">
          <h2 className="text-xl md:text-2xl font-semibold text-white mb-2">
            اختر الاختبار الذي تريد المشاركة فيه
          </h2>
          <p className="text-slate-400 text-sm md:text-base">
            جميع الاختبارات متاحة للمشاركة المباشرة
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-slate-800/50 rounded-lg border border-dashed border-slate-700">
            ...
            <h3 className="text-lg md:text-xl font-semibold text-white mb-2">لا توجد اختبارات متاحة حالياً</h3>
            <p className="text-slate-400 text-sm">سيتم إضافة اختبارات جديدة قريباً.</p>
          </div>
        ) : (
              ...
              >
                <Card className="h-full bg-gradient-to-br from-slate-800/80 to-slate-900/80 border-slate-700 text-white hover:border-blue-500/50 transition-all duration-300 backdrop-blur-sm shadow-lg hover:shadow-xl">
                  ...
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base font-bold text-center mb-2 text-blue-300">
                      {exam.title}
                    </CardTitle>
                    {exam.description && (
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-3">
                      ...
                      <div className="pt-3 border-t border-slate-700">
                        <Button
                          className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold text-sm md:text-base transition-all duration-300"
                          onClick={(e) => {
                            e.stopPropagation();
      ...
      <AlertDialog open={showDashboardLogin} onOpenChange={setShowDashboardLogin}>
        <AlertDialogContent className="bg-slate-900 border-slate-700 text-white">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-transparent hover:bg-slate-700" onClick={() => setPin('')}>
              إلغاء
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleDashboardLogin} className="bg-blue-600 hover:bg-blue-700">
              دخول
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
